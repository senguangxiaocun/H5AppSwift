      ...
      <div v-if="comments.length > 0" class="comment-list">
        <div v-for="(item, index) in comments" :key="index" class="comment-item">
          <div class="comment-top">
            ...
            <img
              @click="sendShowToastToIOS(item.userId)"
              v-if="item.userId !== currentUserStore.currentUser.userId"
              class="report-btn"
              src="@/assets/postpiccommentreport.png"
              alt="report"
            />
          </div>
...
.comment {
  ...
  background: /* 上层：从上到下白到黑渐变 */ linear-gradient(
      to bottom,
      rgba(255, 255, 255, 0),
      rgb(255, 255, 255) 50%
    ),
    /* 下层：从左到右绿到红渐变 */
      linear-gradient(to right, rgba(234, 255, 237, 1), rgba(235, 255, 161, 1));
...
.comment-list {
  ...
  overflow-y: auto;
  max-height: calc(
    calc(100vh * 508 / 812) - calc(100vh * 63 / 812) - calc(100vh * 90 / 812)
  ); /* subtract header and bottom spacing */
}
