<template>
  <div class="page">
        ...
        <div class="top-avatar" :style="{ '--avatar-url': `url(${currentUser.avator})` }">
          <div
            ...
            v-if="
              ...
              !currentUserStore.currentUser.follow.includes(userId)
            "
        ...
        <template v-if="userPosts.length > 0">
          ...
          >
            <div class="post-content">
                ...
                <div class="post-user">
                  <div class="post-avatar">
                    <div
                      ...
                      :style="{ backgroundImage: `url(${currentUser.avator})` }"
                    ></div>
                  </div>
                  <div class="post-username" :title="currentUser.name">
                    {{ currentUser.name }}
                  </div>
                </div>
                <div
              ...
              <div
                class="post-image"
                :style="{ backgroundImage: `url(${post.dynamicPic[0]})` }"
              >
                <div class="post-image-overlay">
                  <div class="overlay-item">
                    <img
                      v-if="
                        currentUserStore.currentUser.postLikeIds.includes(post.dynamicId)
                      "
    ...
    <div class="top-btn">
      ...
      <MoreButton
        v-if="userId !== currentUserStore.currentUser.userId"
        @click="sendShowToastToIOS(userId)"
      />
    </div>
...
function handleChat() {
  if (
    !(
      currentUserStore.currentUser.follow.includes(userId) &&
      userStore.getUserById(userId).follow.includes(currentUserStore.currentUser.userId)
    )
  ) {
    sendShowToastToIOS("nochat");
    return;
  }
  if (uiStore.loading) return;
  uiStore.showLoading();
...
.avatar-bg {
  ...
  opacity: 1;
  background-color: rgba(201, 238, 64, 1);
  pointer-events: none;
}
...
.top-avatar {
  ...
  background: rgba(255, 255, 255, 1); /* 外渐变 */
...
.chat-icon {
  ...
  background-image: url("@/assets/chaticon2.png");
  background-size: cover;
  background-position: center;
}
...
.post-type {
  padding: 0px calc(100vw * 50 / 375) 0px calc(100vw * 10 / 375);
}
